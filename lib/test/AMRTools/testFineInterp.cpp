...
main(int argc ,char* argv[])
{
  ...
  int status = (SpaceDim <= 3) ? testFineInterp() : 0;
  pout() << indent << pgmname ;
  if ( status == 0 )
    {
      pout() << " passed." << endl;
...
testFineInterp()
{
  ...
  // first try with default (no limiting at domain boundaries)
  FineInterp interpolator(fine_grids,
                          num_comps,
                          ref_ratio,
                          fine_problem_domain);
  ...
  //interpolator.m_boundary_limit_type = FineInterp::PCInterp;

  // turn on limiting at domain boundaries.
  //  interpolator.m_boundary_limit_type = FineInterp::limitSlopes;


  const IntVect ghost_vect = IntVect::Zero;
...
// test piecewise linear interpolation
  ...
  const int dest_comp = 0;
           ...
           interpolator.interpToFine(fine_data,
                                     coarse_data);

           fine_data.exchange(fine_data.interval());
//
// compare with exact
           LevelData<FArrayBox> fine_errs(fine_grids, num_comps, IntVect::Zero);
  initData(fine_errs,
           IntVect::Zero,
           fine_dx);
  ...
  if (err_sum < err_sum_tol)
    {
      if (verbose)
        pout() << indent << pgmname << ": FineInterp coarse-fine interpolation test passed." << endl;
      return 0 ;
    }
  else
    {
      pout() << indent << pgmname << ": FineIterp coarse-fine interpolation test FAILED!!!" << endl;
    if (verbose)
    {
